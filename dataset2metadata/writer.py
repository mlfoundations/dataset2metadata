    ...
    def write(self, out_dir_path):
        try:
            for k in self.feature_store:
                self.feature_store[k.replace('-', '_')] = self._flatten_helper(self.feature_store[k], to_npy=True)

            for k in self.parquet_store:
                self.parquet_store[k.replace('-', '_')] = self._flatten_helper(self.parquet_store[k])
